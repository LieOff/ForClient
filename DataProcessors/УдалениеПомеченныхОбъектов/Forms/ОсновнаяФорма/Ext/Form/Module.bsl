...
Процедура ПометкаПриИзменении(Элемент)
	...
	ТекущиеДанные = Items.MarkedForDeletionList.CurrentData;
...
Процедура КомандаСписокПомеченныхУстановитьВсе()
	...
	ЭлементыСписка = MarkedForDeletionList.GetItems();
...
Процедура ИзменитьОбъект(Команда)
	...
	Если ТекущийЭлемент <> Items.MarkedForDeletionList И ТекущийЭлемент <> Элементы.ДеревоОставшихсяОбъектов Тогда
		Возврат;	
	КонецЕсли;
...
Процедура ВыполнитьУдаление()
    ...
    //Если DeletionMode = "Full" Тогда
    //	Состояние(НСтр("en='Searching and delete marked objects';ru='Выполняется поиск и удаление помеченных объектов';cz='Vyhledávání a odstranění označených objektů'"));
    //Иначе
    //	Состояние(НСтр("en='Removing the selected objects';ru='Выполняется удаление выбранных объектов';cz='Removing the selected objects'"));
    //КонецЕсли;
...
Процедура ОбновитьСписокПомеченныхНаУдаление(Команда)
	...
	//Состояние(НСтр("en='Searching for marked for deletion objects';ru='Выполняется поиск помеченных на удаление объектов';cz='Vyhledávání označených k odstranění objektů'"));
	
	ЗаполнениеДереваПомеченныхНаУдаление();
	
	Если КоличествоУровнейПомеченныхНаУдаление = 1 Тогда
		Для Каждого Элемент Из MarkedForDeletionList.GetItems() Цикл
			Идентификатор = Элемент.ПолучитьИдентификатор();
			Items.MarkedForDeletionList.Expand(Идентификатор, False);
		КонецЦикла;
	КонецЕсли;
...
Процедура ОбновитьСодержание(Результат, СообщениеОбОшибке, ТипыУдаленныхОбъектов)
	...
	Если КоличествоНеУдаленныхОбъектов = 0 Тогда
		Если УдаленныхОбъектов = 0 Тогда
			Текст = НСтр("en='No one object marked for deletion';ru='Не помечено на удаление ни одного объекта. Удаление объектов не выполнялось.';cz='No one object marked for deletion'");
			ОбновитьДеревоПомеченных = Ложь;
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			             НСтр("en='Deletion comlete succesfully. "
"Objects deleted: %1.';ru='Удаление помеченных объектов успешно завершено. "
"Удалено объектов: %1.';cz='Odstranění proběhlo úspěšně. "
"Odstraněno objektů: %1.'"),
			             УдаленныхОбъектов);
		КонецЕсли;
		Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.ВыборРежимаУдаления;
		Message(Текст);
...
Процедура ОбновитьДоступностьКнопок()
	...
	Если ТекущаяСтраница = Элементы.ВыборРежимаУдаления Тогда
		...
		Если DeletionMode = "Full" Тогда
			Items.Forward.Enabled = False;
			Items.CommandDelete.Enabled = True;
		ИначеЕсли DeletionMode = "Selectively" Тогда
			Items.Forward.Enabled 	= True;
...
Процедура ЗаполнениеДереваПомеченныхНаУдаление()
	...
	ДеревоПомеченных = РеквизитФормыВЗначение("MarkedForDeletionList");
	...
	ЗначениеВРеквизитФормы(ДеревоПомеченных, "MarkedForDeletionList");
...
Функция УдалениеВыбранныхНаСервере(ТипыУдаленныхОбъектов)
	...
	ПараметрыУдаления = Новый Структура("СписокПомеченныхНаУдаление, РежимУдаления, ТипыУдаленныхОбъектов, ", 
		MarkedForDeletionList, DeletionMode, ТипыУдаленныхОбъектов);
...
Процедура ЗаполнитьСтрокуРезультатов(КоличествоУдаляемых)
	...
	Если УдаленныхОбъектов = 0 Тогда
		СтрокаРезультатов = НСтр("en=""No one object's been deleted, because there are references to the deletion object"";ru='Не удален ни один из объектов, так как в информационной базе существуют ссылки на удаляемые объекты';cz=""No one object's been deleted, because there are references to the deletion object""");
	Иначе
		СтрокаРезультатов = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("en=""Deletion of marked objects is finished."
"Objects's been deleted: %1."";ru='Удаление помеченных объектов завершено."
"Удалено объектов: %1.';cz='Odstranění označených objektů proběhlo úspěšně. "
"Odstraněno objektů: %1.'"),
	...
	Если КоличествоНеУдаленныхОбъектов > 0 Тогда
		СтрокаРезультатов = СтрокаРезультатов + Символы.ПС +
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("en=""Objects aren't deleted: %1."
"Objects hasn't been removed to maintain the integrity of the information base."
"Press OK to view a list of the remaining (non-deleted) objects."";ru='Не удалено объектов: %1."
"Объекты не удалены для сохранения целостности информационной базы, т.к. на них еще имеются ссылки."
"Нажмите ОК для просмотра списка оставшихся (не удаленных) объектов.';cz=""Objects aren't deleted: %1."
"Objects hasn't been removed to maintain the integrity of the information base."
"Press OK to view a list of the remaining (non-deleted) objects."""),
