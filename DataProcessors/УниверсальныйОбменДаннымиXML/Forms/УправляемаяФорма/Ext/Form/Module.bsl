...
Процедура ПриОткрытии(Отказ)
	...
	Если СохраненныйРежимЗагрузки
		...
		Если Объект.НастройкаАвтоматическойЗагрузкиДанных = 1 Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ПриОткрытииЗавершение", ThisForm);
...
Процедура ИмяФайлаПравилПриИзменении(Элемент)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ИмяФайлаПравилПриИзмененииЗавершение", ThisForm);
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьИнформациюОФайлеОбменаЗавершение", ThisForm);
...
Процедура УдалениеУдалить(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("УдалениеУдалитьЗавершение", ThisForm);
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("НастройкаОтладкиЗагрузкиЗавершение", ThisForm);
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПрочитатьПравилаОбменаЗавершение", ThisForm);
...
Процедура ВыборФайла(Элемент, ИмяСвойства, ПроверятьСуществование=Ложь, Знач РасширениеПоУмолчанию = "xml",
	...
	ДиалогВыбораФайла.Заголовок = НСтр("en='Select file';ru='Выберите файл';cz='Zvolit soubor'");
...
Функция ВыполнитьПодключениеКИБПриемникуНаСервере()
	...
	Если РезультатПодключения <> Неопределено Тогда
		...
		СообщитьПользователю(НСтр("en='The connection is successfully established.';ru='Подключение успешно установлено.';cz='The connection is successfully established.'"));
...
Процедура ВыполнитьЗагрузкуИзФормы()
	...
	Если ЭтоКлиент Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьЗагрузкуИзФормыЗавершение", ThisForm);
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,Нстр("ru = 'Файл обмена'"),, УникальныйИдентификатор);
...
Процедура ОткрытьФормуНастройкиОтладкиОбработчиков(ОбработчикиСобытийИзФайлаПравил)
	...
	Обработчик = Новый ОписаниеОповещения("ОткрытьФормуНастройкиОтладкиОбработчиковЗавершение", ThisForm, ОбработчикиСобытийИзФайлаПравил);
	ПараметрыОтладки = ОткрытьФорму(ИмяВызываемойФормы, ПараметрыФормы, ,,,,Обработчик, Режим);
