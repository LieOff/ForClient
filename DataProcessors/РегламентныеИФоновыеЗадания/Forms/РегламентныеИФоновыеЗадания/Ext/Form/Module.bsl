...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Users.IsFullRightUser(Истина) Тогда
...
Процедура ТаблицаРегламентныеЗаданияПередУдалением(Элемент, Отказ)
	...
	Иначе
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ТаблицаРегламентныеЗаданияПередУдалениемЗавершение", ЭтаФорма)
...
&НаКлиенте
Процедура ТаблицаРегламентныеЗаданияПередУдалениемЗавершение(Результат, Параметр)
...
Процедура ВыполнитьРегламентноеЗаданиеВручную(Команда)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ТекущиеДанные = Неопределено Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Выберите регламентное задание.'");
		Сообщение.Сообщить();
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Если КоличествоОшибок > 5 Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ВыполнитьРегламентноеЗаданиеВручнуюЗавершение", ЭтаФорма, ТекстВсехОшибок)
...
&НаКлиенте
Процедура ВыполнитьРегламентноеЗаданиеВручнуюЗавершение(Результат, Параметр) Экспорт
...
Процедура ЗапуститьОтдельныйСеанс(Команда)
	...
	Состояние(НСтр("ru = 'Запуск отдельного сеанса для выполнения регламентных заданий.'"),
	          ,
	          НСтр("en='Please, wait...';ru='Пожалуйста, подождите...';cz='Please, wait...'"));
...
Процедура ОткрытьРасписаниеРегламентногоЗаданияВыполнить(Команда)
	...
	Иначе
		...
		Диалог.Показать(Новый ОписаниеОповещения("ОткрытьРасписаниеРегламентногоЗаданияВыполнитьЗавершение", ЭтаФорма, ТекущиеДанные.Идентификатор));
...
&НаКлиенте
Процедура ОткрытьРасписаниеРегламентногоЗаданияВыполнитьЗавершение(Результат, Параметр) Экспорт
	
	Если Не Результат = Неопределено Тогда
		
		УстановитьРасписание(Параметр, Результат);
